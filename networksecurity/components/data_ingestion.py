...
class DataIngestion:
    ...
    def export_collection_as_dataframe(self):
        ...
        try:
            ...
            if df.empty:  # ✅ Check if DataFrame is empty
                raise ValueError("No data found in MongoDB collection. Ensure the database is populated.")
    ...
    def split_data_as_train_test(self, dataframe: pd.DataFrame):
        try:
            ...
            logger.info(f"Exporting train and test file paths.")

            train_set.to_csv(self.data_ingestion_config.training_file_path, index=False, header=True)
            test_set.to_csv(self.data_ingestion_config.testing_file_path, index=False, header=True)
            logger.info(f"Exported train and test file paths.")
